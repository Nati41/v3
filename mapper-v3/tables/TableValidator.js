...
export function validateTableBBox(bbox) {
    ...
    // Table bbox should be larger than header - at least 10% or 100px
    const minWidth = isNormalized ? 0.1 : 100;   // 10% or 100px
    const minHeight = isNormalized ? 0.1 : 100;  // 10% or 100px

    if (bbox.width < minWidth) {
        return { valid: false, error: 'הטבלה צרה מדי (מינימום 100 פיקסלים)' };
    }

    if (bbox.height < minHeight) {
        return { valid: false, error: 'הטבלה נמוכה מדי (מינימום 100 פיקסלים)' };
    }
