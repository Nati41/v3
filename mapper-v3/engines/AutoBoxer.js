...
export class AutoBoxer {
    ...
    setNeighborFields(fields) {
        ...
        console.log(`[AutoBoxer] Set ${this._neighborFields.length} neighbor fields as walls:`, this._neighborFields);
    }
    ...
    async computeBbox(clickX, clickY) {
        console.log(`[AutoBoxer] Computing bbox at click (${clickX}, ${clickY}), neighborFields: ${this._neighborFields?.length || 0}`);
        ...
        console.log(`[AutoBoxer] ✓ Floor found at Y=${floor.y}, length=${floor.length}, extent=[${floor.leftExtent}, ${floor.rightExtent}]`);
        ...
        console.log(`[AutoBoxer] Left wall: ${leftWall ? `found at X=${leftWall.x}` : 'not found (using default)'}, using X=${leftX} (floor bound=${floorLeftBound})`);
        ...
        console.log(`[AutoBoxer] Right wall: ${rightWall ? `found at X=${rightWall.x}` : 'not found (using default)'}, using X=${rightX} (floor bound=${floorRightBound})`);
        ...
        if (computedWidth > maxWidth) {
            console.log(`[AutoBoxer] Width ${Math.round(computedWidth)} exceeds MAX_WIDTH ${Math.round(maxWidth)}, centering on click`);
    ...
    _findLeftWall(startX, floorY, clickX) {
        ...
        // V3.2: Apply priority rules
        // Priority: 1. Existing fields  2. Vertical lines  3. Text (fallback only if no line)

        // RULE: If vertical line exists, use it (ignore text)
        // This ensures precise wall detection at actual field boundaries
        if (verticalLineWall) {
            // Check if existing field is closer
            if (fieldWall && fieldWall.distance < verticalLineWall.distance) {
                console.log(`[AutoBoxer] Left wall: field at X=${fieldWall.x}`);
                return { x: fieldWall.x };
            }
            console.log(`[AutoBoxer] Left wall: vertical_line at X=${verticalLineWall.x}`);
            return { x: verticalLineWall.x };
        }

        // No vertical line - use text or field (whichever is closer)
        if (textWall && fieldWall) {
            const winner = textWall.distance < fieldWall.distance ? textWall : fieldWall;
            console.log(`[AutoBoxer] Left wall: ${winner === textWall ? 'text' : 'field'} at X=${winner.x}`);
            return { x: winner.x };
        }

        if (textWall) {
            console.log(`[AutoBoxer] Left wall: text at X=${textWall.x}`);
            return { x: textWall.x };
        }
    ...
    _findRightWall(startX, floorY, clickX) {
        ...
        if (verticalLineWall) {
            // Check if existing field is closer
            if (fieldWall && fieldWall.distance < verticalLineWall.distance) {
                console.log(`[AutoBoxer] Right wall: field at X=${fieldWall.x}`);
                return { x: fieldWall.x };
            }
            console.log(`[AutoBoxer] Right wall: vertical_line at X=${verticalLineWall.x}`);
            return { x: verticalLineWall.x };
        }
        ...
        if (textWall && fieldWall) {
            const winner = textWall.distance < fieldWall.distance ? textWall : fieldWall;
            console.log(`[AutoBoxer] Right wall: ${winner === textWall ? 'text' : 'field'} at X=${winner.x}`);
            return { x: winner.x };
        }

        if (textWall) {
            console.log(`[AutoBoxer] Right wall: text at X=${textWall.x}`);
            return { x: textWall.x };
        }
    ...
    findAllWalls(startX, floorY, direction, floorBounds) {
        ...
        console.log(`[AutoBoxer] findAllWalls ${direction}: ${walls.length} walls, hardLimit=${Math.round(hardLimit)}`);
    ...
    _isIsolatedVerticalLine(x, floorY, minHeight) {
        ...
        // ============ CHECK 0.5: CEILING CONNECTION (DISABLED FOR TESTING) ============
        // V3.2: Temporarily disabled to test if this causes precision issues
        // if (this._hasConnectedCeiling(x, floorY, minHeight)) {
        ...
        // console.log(`[AutoBoxer] Line X=${x}: neighbors L=${leftSimilarCount} R=${rightSimilarCount}`);
    ...
    _hasConnectedCeiling(x, floorY, minHeight) {
        ...
        if (hasCeiling) {
            console.log(`[AutoBoxer] Line X=${Math.round(x)} has ceiling (L=${leftInkCount}, R=${rightInkCount}) -> STRUCTURAL`);
        }
    ...
    _findExistingFieldEdge(startX, floorY, direction) {
        ...
        for (const field of this._neighborFields) {
            ...
            // Check vertical overlap (field must be on same horizontal band)
            const verticalOverlap = screenFloorY >= fieldTop - 15 && screenFloorY <= fieldBottom + 15;
            if (!verticalOverlap) continue;

            if (direction === 'left') {
                // Looking for fields to our LEFT - use their RIGHT edge as wall
                if (fieldRight < screenStartX) {
                    const wallScreenX = fieldRight + PADDING;
                    const distance = screenStartX - wallScreenX;
                ...
                // Looking for fields to our RIGHT - use their LEFT edge as wall
                if (fieldLeft > screenStartX) {
                    const wallScreenX = fieldLeft - PADDING;
                    const distance = wallScreenX - screenStartX;
                    if (distance < closestDistance) {
                        closestDistance = distance;
                        closestWall = { x: Math.round(wallScreenX * this._scale) };
                    }
        ...
        if (closestWall) {
            console.log(`[AutoBoxer] Found existing field as ${direction} wall at X=${closestWall.x}`);
        }
    ...
    /**
     * V3.2: Check if an existing field blocks expansion at this X position (LEGACY - kept for compatibility)
     * Treats existing fields (blue rectangles) as walls
     *
     * @param {number} x - Canvas X coordinate
     * @param {number} floorY - Floor Y coordinate in canvas pixels
     * @param {string} direction - 'left' or 'right' (which direction we're scanning)
     * @returns {Object|null} { x: wallX } if field boundary found, null otherwise
     */
    _isExistingFieldWall(x, floorY, direction) {
        if (!this._neighborFields || this._neighborFields.length === 0) {
            return null;
        }
        ...
        for (const field of this._neighborFields) {
            ...
            // Check if scan position is at same vertical level as this field
            // (field must be on same horizontal band as our floor)
            const verticalOverlap = screenFloorY >= fieldTop - 10 && screenFloorY <= fieldBottom + 10;
            if (!verticalOverlap) continue;

            if (direction === 'left') {
                // Scanning left - check if we hit the RIGHT edge of an existing field
                if (screenX <= fieldRight + PADDING && screenX >= fieldLeft) {
                    const wallX = (fieldRight + PADDING) * this._scale;
                    console.log(`[AutoBoxer] Existing field wall (left): field ends at X=${fieldRight}, wall at X=${Math.round(wallX)}`);
                    return { x: Math.round(wallX) };
                }
            } else {
                // Scanning right - check if we hit the LEFT edge of an existing field
                if (screenX >= fieldLeft - PADDING && screenX <= fieldRight) {
                    const wallX = (fieldLeft - PADDING) * this._scale;
                    console.log(`[AutoBoxer] Existing field wall (right): field starts at X=${fieldLeft}, wall at X=${Math.round(wallX)}`);
                    return { x: Math.round(wallX) };
                }
            }
        }
