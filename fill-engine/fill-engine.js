...
(function() {
    ...
    function reverseForRTL(text) {
        // Split by words, reverse order, keep numbers LTR
        const words = text.split(/(\s+)/);
        const reversed = words.reverse();

        return reversed.map(word => {
            // Keep numbers and punctuation in original order
            if (/^[\d\s.,\-:\/]+$/.test(word)) {
                return word;
            }
    ...
    async function fillTextField(page, field, value, fonts, options = {}) {
        ...
        // Wrap text if needed
        const lines = wrapText(formattedValue, font, fontSize, pdfCoords.width - FILL_CONFIG.TEXT_PADDING * 2);
        const lineHeight = fontSize * FILL_CONFIG.LINE_HEIGHT_FACTOR;

        // Check for overflow
        const totalHeight = lines.length * lineHeight;
        if (totalHeight > pdfCoords.height) {
            diagnostics.push(createDiagnostic(
                DIAGNOSTIC_TYPES.OVERFLOW,
                field.id,
                'Text exceeds field height',
                { lines: lines.length, height: totalHeight, maxHeight: pdfCoords.height }
            ));
        }
    ...
    async function fillTextFieldDirect(page, field, value, fonts, options = {}) {
        ...
        const y = pdfCoords.y + (pdfCoords.height - fontSize) / 2;
