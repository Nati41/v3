/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - FieldIntentResolver
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   מחליט איך לרנדר שדה: תאים (perGlyphBoxes) או טקסט חופשי (flowText).
 *   למשל: ת.ז. = 9 תאים בודדים, שם = טקסט חופשי.
 *
 * איך זה עובד:
 *   - בודק כוונה (intent), צורת bbox, דפוס ערך, רמז רינדור
 *   - סף ביטחון: גבוה=0.85, בינוני=0.65, נמוך=0.40
 *   - טווח ספרות לתאים: 6-20 תאים רצופים
 *   - גנרי לחלוטין - לא מכיר סמנטיקה של שדות
 *
 * מי משתמש בקובץ:
 *   - fill-engine.js - החלטה על רינדור בייצוא
 *   - preview-text-renderer.js - תצוגה מקדימה ב-LiveFill
 *
 * באיזה מצבים:
 *   מצב מילוי - בכל רינדור של שדה
 *
 * למה הוא קיים:
 *   כדי שגם ה-export וגם ה-preview ירנדרו שדות באותו אופן.
 * ═══════════════════════════════════════════════════════════════
 */
