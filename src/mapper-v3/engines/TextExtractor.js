/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - TextExtractor
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   חילוץ טקסט מתוך שטח מוגדר ב-PDF.
 *   המשתמש מצייר מלבן → הקובץ מחזיר את הטקסט שנמצא בדיוק בתוכו.
 *
 * איך זה עובד:
 *   - קודם: pdf.js getTextContent() - קריאת שכבת טקסט מה-PDF
 *   - חלופה: OCR דרך Tesseract.js - לטפסים סרוקים בלי שכבת טקסט
 *   - מצב STRICT BBOX: רק טקסט שנמצא לגמרי בתוך המלבן
 *   - אין חפיפה חלקית, אין סף סובלנות
 *   - cache לדף - לא קורא שוב מ-PDF
 *
 * מי משתמש בקובץ:
 *   - DrawController.js - חילוץ שם שדה אחרי ציור
 *   - AutoBoxer.js - בדיקת טקסט קיים באזור
 *   - FieldNamer.js - קריאת טקסט לצורך מתן שם
 *
 * באיזה מצבים:
 *   מצב מיפוי - בכל פעם שנוצר שדה חדש
 *
 * למה הוא קיים:
 *   כדי לשלוף אוטומטית את שם השדה מתוך ה-PDF
 *   (למשל: "שם פרטי", "מספר זהות") בלי שהמשתמש יקליד ידנית.
 * ═══════════════════════════════════════════════════════════════
 */
