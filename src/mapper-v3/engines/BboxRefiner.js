/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - BboxRefiner
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   שיפור הדרגתי של מלבני שדות (bbox).
 *   אחרי שה-AutoBoxer מציע מלבן ראשוני, ה-Refiner מאפשר
 *   למשתמש לשפר אותו בלחיצות ובגרירה.
 *
 * איך זה עובד:
 *   - מייצר "מועמדים" לכל צלע של המלבן:
 *     שמאל/ימין: קיר1, קיר2, דילוג מכשולים, יישור לשכן, קצה דף
 *     למעלה: תקרה1, תקרה2, תחתית תווית, גובה ברירת מחדל
 *     למטה: רצפה1, רצפה מגושרת, תחתית שורה
 *   - לחיצה בחוץ → הרחבה לכיוון הלחיצה
 *   - לחיצה בפנים ליד צלע → כיווץ
 *   - גרירה → כיוונון עדין (fine-tune)
 *   - לחיצה במרכז → הזזה
 *   - מניעת התנגשות: לא חוצה גבולות טקסט או שדות קיימים
 *
 * מי משתמש בקובץ:
 *   - DrawController.js - מפעיל refine() אחרי AutoBoxer
 *
 * באיזה מצבים:
 *   - מצב מיפוי - בכל פעם שמשתמש מצייר שדה חדש
 *
 * למה הוא קיים:
 *   AutoBoxer נותן מלבן ראשוני טוב, אבל לפעמים לא מושלם.
 *   ה-Refiner מאפשר שיפור בלחיצה אחת נוספת במקום גרירה ידנית.
 *
