/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - WordSelector
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   מאפשר למשתמש לבחור מילים מתוך טקסט ה-PDF לבניית שם שדה.
 *   המשתמש לוחץ על מילים אחת אחת → הן מצטרפות לשם השדה.
 *
 * איך זה עובד:
 *   - שולף מילים משכבת הטקסט של pdf.js (ללא OCR)
 *   - מסווג מילים: ניתנות לבחירה / לא (מספרים בודדים, סימנים)
 *   - סדר לחיצה = סדר השם (לא סדר מרחבי)
 *   - תומך טקסט רב-שורתי, גלישה, כל פריסה
 *   - דפוסי עברית: 2+ תווים, מילות תאריך, מספרים
 *
 * מי משתמש בקובץ:
 *   - DrawController.js - מצב בחירת שם שדה
 *   - LabelOverlay.js - תצוגת מילים שנבחרו
 *
 * באיזה מצבים:
 *   מצב מיפוי - כשמשתמש בוחר שם לשדה מתוך ה-PDF
 *
 * למה הוא קיים:
 *   במקום שהמשתמש יקליד את שם השדה, הוא פשוט לוחץ
 *   על המילים שכבר כתובות בטופס.
 * ═══════════════════════════════════════════════════════════════
 */
