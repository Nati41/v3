/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - PDFEngine
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   רינדור טפסי PDF לתצוגה על המסך.
 *   טוען קובץ PDF בעזרת ספריית pdf.js, מרנדר אותו ב-300 DPI
 *   על Canvas, ממיר ל-PNG, ומציג כתמונה (<img>).
 *
 * איך זה עובד:
 *   - PDF → Canvas (300 DPI) → PNG DataURL → <img> element
 *   - תצוגה כ-<img> ולא כ-<canvas> לשיפור ביצועים
 *   - cache לכל דף - מעבר בין דפים מיידי
 *   - מגדיר pdfPageDimensions מוקדם לפני שה-Overlay מרנדר
 *
 * מי משתמש בקובץ:
 *   - MapperCore.js - טעינת PDF ומעבר בין דפים
 *   - AutoBoxer.js - קריאת פיקסלים מה-canvas
 *   - DrawController.js - קביעת מימדי דף
 *   - OverlayRenderer.js - התאמת שכבה לגודל ה-PDF
 *
 * באיזה מצבים:
 *   כל המצבים - תמיד פעיל כשיש PDF טעון
 *
 * למה הוא קיים:
 *   המנוע שמציג את ה-PDF שעליו עובדים. בלעדיו אין מה לראות.
 * ═══════════════════════════════════════════════════════════════
 */
