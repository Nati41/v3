/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - DragController
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   מאפשר גרירה ושינוי גודל של שדות מוגדרים.
 *   המשתמש לוחץ על שדה וגורר אותו למיקום חדש,
 *   או גורר את הפינה לשינוי גודל.
 *
 * איך זה עובד:
 *   - מעקב אחרי mousedown/mousemove/mouseup
 *   - מצב גרירה: מזיז את השדה (שינוי x,y)
 *   - מצב שינוי גודל: משנה מימדים (width, height)
 *   - Frame limiting דרך requestAnimationFrame
 *   - פורט מדויק מ-V2 (drag-engine.js) עם אותן נוסחאות
 *
 * מי משתמש בקובץ:
 *   - MapperCore.js - מאתחל ומחבר אירועי עכבר
 *   - OverlayRenderer.js - מעדכן תצוגה בזמן גרירה
 *
 * באיזה מצבים:
 *   מצב מיפוי - כשיש שדות שאפשר לגרור
 *
 * למה הוא קיים:
 *   כדי שהמשתמש יוכל לכוונן מיקום וגודל של שדות בגרירה.
 * ═══════════════════════════════════════════════════════════════
 */
...
export class DragController {
    ...
    _setupListeners() {
        ...
        // PERFORMANCE: Frame-limited mousemove - prevents jank during drag/resize
        document.addEventListener('mousemove', (e) => {
            // Early exit if not active - minimal overhead
