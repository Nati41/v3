/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - OverlayRenderer
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   מציג שכבה ויזואלית של מלבנים צבעוניים מעל ה-PDF.
 *   כל שדה שמופה מקבל אוברליי שניתן ללחוץ, לגרור ולשנות גודל.
 *
 * איך זה עובד:
 *   - מנהל שכבת DOM (#overlay-layer) מעל תמונת ה-PDF
 *   - כל שדה = אלמנט <div> עם מיקום absolute
 *   - תומך ב-ghost overlays (מלבנים "רפאים") לשדות תבנית שעוד לא מופו
 *   - המרת קואורדינטות: bbox מנורמל (0-1) → פיקסלים CSS
 *   - שמירת cache לקואורדינטות מסך (v3.9)
 *
 * מי משתמש בקובץ:
 *   - MapperCore.js - אתחול השכבה
 *   - DrawController.js - הצגת שדה חדש אחרי ציור
 *   - DragController.js - עדכון מיקום בזמן גרירה
 *   - SidebarController.js - הדגשת שדה נבחר
 *   - GuidedMappingUI.js - הצגת ghost overlays
 *
 * באיזה מצבים:
 *   כל המצבים - תמיד פעיל כשיש שדות מוגדרים
 *
 * למה הוא קיים:
 *   זו השכבה שמראה למשתמש מה כבר מיפה.
 *   בלעדיה, המשתמש לא רואה את השדות על ה-PDF.
 * ═══════════════════════════════════════════════════════════════
 */
