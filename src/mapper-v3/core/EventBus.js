/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - EventBus
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   מערכת אירועים מרכזית - כל התקשורת בין מודולים עוברת כאן.
 *   מודול אחד "פולט" (emit) אירוע, אחרים "מאזינים" (on) לו.
 *   אין תלויות ישירות בין מודולים.
 *
 * איך זה עובד:
 *   - eventBus.emit('DRAW_END', data) - שליחת אירוע
 *   - eventBus.on('DRAW_END', callback) - האזנה לאירוע
 *   - 50+ סוגי אירועים: ui:, field:, template:, table:, radio:, ai:, export:
 *   - מוניטור ביצועים עם מצב חירום (מונע קריסת דפדפן)
 *
 * מי משתמש בקובץ:
 *   כל מודול בפרויקט
 *
 ...
 *   תמיד פעיל
 *
 * Singleton: export const eventBus
 * מייצא גם: Events (אובייקט עם כל סוגי האירועים)
 * ═══════════════════════════════════════════════════════════════
 */
