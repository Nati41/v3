...
class FieldMapper {
    ...
    // ============ LAYOUT MODE CONTROLS (Phase 2) ============

    /**
     * Set field layout mode (flow or slots).
     * Uses LayoutHelper for safe, consistent layout creation.
     * @param {string} fieldId - Field ID
     * @param {string} mode - 'flow' or 'slots'
     */
    setFieldLayoutMode(fieldId, mode) {
        const field = this.fields.find(f => f.id === fieldId);
        if (!field) {
            console.warn('[Mapper] setFieldLayoutMode: field not found:', fieldId);
            return;
        }

        const LH = window.LayoutHelper;
        if (!LH) {
            console.error('[Mapper] LayoutHelper not available');
            return;
        }

        // Get current layout to preserve slotCount if switching back to slots
        const currentLayout = LH.getFieldLayout(field);
        const previousSlotCount = currentLayout.slotCount || 9;

        // Create new layout using LayoutHelper (safe creation)
        if (mode === 'slots') {
            // When switching to slots, try to auto-detect from bbox if no previous count
            let slotCount = previousSlotCount;
            if (field.bbox && field.bbox[2] && field.bbox[3] && !currentLayout.slotCount) {
                const ratio = field.bbox[2] / field.bbox[3];
                if (ratio >= 4 && ratio <= 25) {
                    slotCount = Math.round(ratio);
                }
            }
            field.layout = LH.createSlotsLayout(slotCount, LH.SOURCES.EXPLICIT);
        } else {
            field.layout = LH.createFlowLayout(LH.OVERFLOW.SHRINK, LH.SOURCES.EXPLICIT);
        }

        console.log(`[Mapper] Layout set: ${fieldId} → ${mode}`, field.layout);

        // Update UI and save state
        this.updateFieldList();
        this.saveState('update_layout');
        this.showToast(`מצב תצוגה: ${mode === 'slots' ? 'תיבות' : 'טקסט'}`, 'success');
    }

    /**
     * Set slot count for a field in slots mode.
     * @param {string} fieldId - Field ID
     * @param {number} slotCount - Number of slots (1-30)
     */
    setFieldSlotCount(fieldId, slotCount) {
        const field = this.fields.find(f => f.id === fieldId);
        if (!field) {
            console.warn('[Mapper] setFieldSlotCount: field not found:', fieldId);
            return;
        }
        ...
        // Ensure we're in slots mode, then update
        const currentLayout = LH.getFieldLayout(field);
        if (currentLayout.mode !== LH.MODES.SLOTS) {
            // If not in slots mode, switch to it
            field.layout = LH.createSlotsLayout(slotCount, LH.SOURCES.EXPLICIT);
        } else {
            // Update existing layout's slotCount
            field.layout = LH.createSlotsLayout(slotCount, LH.SOURCES.EXPLICIT);
        }

        console.log(`[Mapper] Slot count: ${fieldId} → ${slotCount}`);

        // Update UI and save state
        this.updateFieldList();
        this.saveState('update_slot_count');
    }

    /**
     * Adjust slot count by delta (+1 or -1).
     * @param {string} fieldId - Field ID
     * @param {number} delta - Amount to adjust (+1 or -1)
     */
    adjustFieldSlotCount(fieldId, delta) {
        const field = this.fields.find(f => f.id === fieldId);
        if (!field) {
            console.warn('[Mapper] adjustFieldSlotCount: field not found:', fieldId);
            return;
        }
        ...
        this.setFieldSlotCount(fieldId, currentCount + delta);
    }
