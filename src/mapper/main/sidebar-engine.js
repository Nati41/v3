...
(function() {
    ...
    // ============ LAYOUT TOGGLE HELPER (Phase 2) ============

    /**
     * Render layout mode toggle for a field.
     * Uses LayoutHelper for safe access to layout data.
     * @param {Object} field - Field object
     * @returns {string} HTML string for layout toggle
     */
    function renderLayoutToggle(field) {
        // Use LayoutHelper for safe access (never crashes on missing data)
        const LH = window.LayoutHelper;
        if (!LH) {
            console.warn('[sidebar-engine] LayoutHelper not loaded');
            return '<!-- LayoutHelper not available -->';
        }

        // Get current layout using safe accessor
        const layout = LH.getFieldLayout(field);
        const isSlots = layout.mode === LH.MODES.SLOTS;
        const slotCount = layout.slotCount || 9;
        const source = layout.source || 'default';

        // Source indicator for debugging
        const sourceIcon = source === 'explicit' ? '✋' :
                          source === 'legacy_renderHint' ? '📋' : '🔄';
        const sourceTitle = source === 'explicit' ? 'נקבע ידנית' :
                           source === 'legacy_renderHint' ? 'מנתונים קיימים' : 'ברירת מחדל';

        return `
            <div class="editor-row layout-toggle-row">
                <label>מצב תצוגה <span class="layout-source" title="${sourceTitle}">${sourceIcon}</span></label>
                <div class="layout-toggle">
                    <button type="button"
                            class="${!isSlots ? 'active' : ''}"
                            onclick="mapper.setFieldLayoutMode('${field.id}', 'flow')"
                            title="טקסט זורם - לשמות, כתובות, ערים">
                        📝 טקסט
                    </button>
                    <button type="button"
                            class="${isSlots ? 'active' : ''}"
                            onclick="mapper.setFieldLayoutMode('${field.id}', 'slots')"
                            title="תיבות נפרדות - לת.ז, טלפון, תאריך">
                        🔢 תיבות
                    </button>
                </div>
            </div>
            <div class="editor-row slot-count-row" style="display: ${isSlots ? 'flex' : 'none'}">
                <label>מספר תיבות</label>
                <div class="slot-count-control">
                    <button type="button" onclick="mapper.adjustFieldSlotCount('${field.id}', -1)" ${slotCount <= 1 ? 'disabled' : ''}>−</button>
                    <input type="number" min="1" max="30" value="${slotCount}"
                           onchange="mapper.setFieldSlotCount('${field.id}', parseInt(this.value))"
                           style="width: 50px; text-align: center;">
                    <button type="button" onclick="mapper.adjustFieldSlotCount('${field.id}', 1)" ${slotCount >= 30 ? 'disabled' : ''}>+</button>
                </div>
            </div>
        `;
    }
    ...
    function renderFieldItem(field, isExpanded, isSelected, isInMappingMode) {
        ...
        <div class="field-item ${isSelected ? 'selected' : ''} ${isExpanded ? 'expanded' : ''} ${isUnmapped ? 'unmapped' : ''} ${isLinked ? 'linked' : ''} ${typeClass} ${isGroupingMode && isRadio ? 'grouping-selectable' : ''} ${isSelectedForGroup ? 'selected-for-group' : ''}" data-field-id="${field.id}">
            ...
            <div class="field-editor">
                ...
                <!-- Layout Mode Toggle (Phase 2) -->
                ${renderLayoutToggle(field)}
