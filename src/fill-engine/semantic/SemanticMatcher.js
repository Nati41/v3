/**
 * ═══════════════════════════════════════════════════════════════
 * תיעוד בעברית - SemanticMatcher
 * ═══════════════════════════════════════════════════════════════
 *
 * מה הקובץ עושה:
 *   התאמה סמנטית חכמה בין שמות שדות לנתונים.
 *   גם אם השם שונה ("שם משפחה" ↔ "last_name") - מזהה שזה אותו שדה.
 *
 * איך זה עובד:
 *   5 שכבות התאמה:
 *   1. התאמה מדויקת (case-insensitive) → ביטחון 1.0
 *   2. מילון נרדפות → ביטחון 0.95
 *   3. התאמה מעורפלת (Levenshtein) → ביטחון 0.70-0.85
 *   4. זיהוי לפי פורמט (ת.ז.=9 ספרות, טלפון=10) → ביטחון 0.60
 *   5. הקשר ישות → ביטחון 0.50+
 *
 * מי משתמש בקובץ:
 *   - fill-engine.js - לפני כל מילוי
 *   - ExcelDataResolver.js - התאמת עמודות
 *
 * באיזה מצבים:
 *   מצב מילוי - אוטומטית בכל מילוי
 *
 * למה הוא קיים:
 *   כי שמות שדות בטפסים שונים לא אחידים.
 *   מישהו כותב "שם משפחה", אחר כותב "שם_משפחה", שלישי "last name".
 *   המנוע מזהה שכולם אותו דבר.
 * ═══════════════════════════════════════════════════════════════
 */
